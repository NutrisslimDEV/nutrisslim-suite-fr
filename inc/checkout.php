...
function customize_checkout_fields($fields) {
    ...
    if ($default_country == 'GB') {
        ...
        $fields['billing']['billing_house_number'] = array(
            'type'      => 'text',
            'label'     => __('Numéro de maison', 'nutrisslim-suite'),
            'placeholder' => __('Numéro de maison', 'nutrisslim-suite'),
            'class' => array('form-row-first house_number-field'),
            'required' => 1,
            'priority' => 40,
            'clear'       => true,
        ...
        // Add billing zip code
        $fields['billing']['billing_postcode'] = array(
            'label'       => __('Code postal (ex. 00000)', 'woocommerce'),
            'placeholder' => __('Code postal (ex. 00000) *', 'woocommerce'),
            'required'    => true,
            'class'       => array('form-row-first'),
            'clear'       => true,
            'priority' => 60,
        ...
        jQuery(document).ready(function($) {
            function restrictCityField(selector) {
                $(selector).on('keypress', function(event) {
                    ...
                    // Allow letters (including accented), space, hyphen, apostrophe, comma, period
                    const validChar = /^[\p{L}\p{M}\s\-'\.,]$/u;
                ...
                $(selector).on('paste', function(event) {
                    ...
                    // Allow only valid characters in paste
                    const validPaste = /^[\p{L}\p{M}\s\-'\.,]+$/u;
                    if (!validPaste.test(pasteData)) {
                        event.preventDefault();
                    }
                });
            }
...
function validate_billing_postcode_format() {
    ...
    if ( ! preg_match('/^\d{5}$/', $postcode) ) {
        wc_add_notice(__('Veuillez entrer un code postal valide à cinq chiffres.'), 'error');
    }
}
...
function save_custom_checkout_fields($order_id, $data) {
    ...
    // ----- BILLING -----
    $billing_street = isset($data['billing_address_1']) ? sanitize_text_field($data['billing_address_1']) : '';
    $billing_house  = isset($data['billing_house_number']) ? sanitize_text_field($data['billing_house_number']) : '';
    $billing_extra  = isset($data['billing_address_2']) ? sanitize_text_field($data['billing_address_2']) : '';

    // Force Woo standard: address_1 = house no, address_2 = street
    $order->set_billing_address_1($billing_house);
    $order->set_billing_address_2($billing_street);

    if ($billing_extra) {
        $order->add_meta_data('_billing_address_extra', $billing_extra);
    }
    if ($billing_house) {
    ...
    // ----- SHIPPING -----
    $shipping_street = isset($data['shipping_address_1']) ? sanitize_text_field($data['shipping_address_1']) : '';
    $shipping_house  = isset($data['shipping_house_number']) ? sanitize_text_field($data['shipping_house_number']) : '';
    $shipping_extra  = isset($data['shipping_address_2']) ? sanitize_text_field($data['shipping_address_2']) : '';

    $order->set_shipping_address_1($shipping_house);
    $order->set_shipping_address_2($shipping_street);

    if ($shipping_extra) {
        $order->add_meta_data('_shipping_address_extra', $shipping_extra);
    }
    if ($shipping_house) {
...
//Paypal fix house number functions

/**
 * Treat only PayPal orders (PPEC/PPCP/etc.) as targets.
 */
function ns_is_paypal_like_gateway( $gateway_id ) {
    $gateway_id = (string) $gateway_id;

    // Common PayPal IDs across plugins (extendable via filter)
    $paypal_ids = apply_filters('ns_paypal_gateway_ids', [
        'ppec_paypal',                // WooCommerce PayPal Express Checkout
        'paypal_express',             // some forks
        'paypal',                     // legacy PayPal Standard
        'ppcp',                       // WooCommerce PayPal Payments (PPCP)
        'ppcp-gateway',               // credit-card gateway id
        'ppcp-paypal',                // smart buttons id
    ...
    // Fallback heuristics
    if ( $gateway_id !== '' && (strpos($gateway_id, 'paypal') !== false || strpos($gateway_id, 'ppcp') !== false) ) {
        return true;
    }
...
/**
 * Normalize order addresses to:
 *   address_1 = house number
 *   address_2 = street
 */
function ns_extract_house_and_street_from_pair( $a1, $a2, $custom_house = '' ) {
    $a1 = trim( (string) $a1 );
    $a2 = trim( (string) $a2 );
    $custom_house = trim( (string) $custom_house );

    // If custom house field exists, trust it
    if ( $custom_house !== '' ) {
        $house  = $custom_house;
        // Street is whichever field is not the house
        $street = $a2 !== '' ? $a2 : $a1;
        return [ $house, trim($street) ];
    }

    // If address_1 ends with a number -> "Street Name 23"
    if ( preg_match('/^(.*?)[,\s]+(\d+[A-Za-z]?([\/-]\d+[A-Za-z]?)?)\s*$/u', $a1, $m) ) {
        return [ trim($m[2]), trim($m[1]) ];
    }

    // If address_2 looks like a pure house number -> use that
    if ( preg_match('/^\s*\d+[A-Za-z]?([\/-]\d+[A-Za-z]?)?\s*$/u', $a2) ) {
        return [ trim($a2), $a1 ];
    }

    // If address_1 is just a number -> house in a1, street in a2
    if ( preg_match('/^\s*\d+[A-Za-z]?([\/-]\d+[A-Za-z]?)?\s*$/u', $a1) ) {
        return [ $a1, $a2 ];
    }

    // Couldn’t determine — leave unchanged
    return [ '', '' ];
}

function ns_normalize_order_addresses_core( WC_Order $order, array $data = [] ) {
    // BILLING
    $b_house_custom = $data['billing_house_number'] ?? ( $_POST['billing_house_number'] ?? '' );
    $b_a1 = $order->get_billing_address_1();
    $b_a2 = $order->get_billing_address_2();
    [ $b_house, $b_street ] = ns_extract_house_and_street_from_pair( $b_a1, $b_a2, $b_house_custom );
    if ( $b_house !== '' || $b_street !== '' ) {
        $order->set_billing_address_1( $b_house );
        $order->set_billing_address_2( $b_street );
        if ( $b_house !== '' ) {
            $order->update_meta_data( '_billing_house_number', $b_house );
        }
    }

    // SHIPPING
    $s_house_custom = $data['shipping_house_number'] ?? ( $_POST['shipping_house_number'] ?? '' );
    $s_a1 = $order->get_shipping_address_1();
    $s_a2 = $order->get_shipping_address_2();
    [ $s_house, $s_street ] = ns_extract_house_and_street_from_pair( $s_a1, $s_a2, $s_house_custom );
    if ( $s_house !== '' || $s_street !== '' ) {
        $order->set_shipping_address_1( $s_house );
        $order->set_shipping_address_2( $s_street );
        if ( $s_house !== '' ) {
            $order->update_meta_data( '_shipping_house_number', $s_house );
        }
    }
...
/** Only normalize if order uses a PayPal(-like) gateway */
function ns_should_normalize_paypal_order( WC_Order $order ): bool {
    $gid = (string) $order->get_payment_method();
    return ns_is_paypal_like_gateway( $gid );
}

/** Run after payment — catches PayPal Express overwrites */
add_action( 'woocommerce_payment_complete', function( $order_id ) {
    $order = wc_get_order( $order_id );
    if ( ! $order || ! ns_should_normalize_paypal_order( $order ) ) {
        return;
    }
    ns_normalize_order_addresses_core( $order );
}, 1 );

/** Final guard on thank-you (also runs for offsite gateways callbacks) */
...
function get_checkout_offer_ajax() {
        ...
        foreach ($items as $item) {
            ...
            echo '<img decoding="async" src="/wp-content/uploads/2024/04/Home-Ikone-NOVO_cart.svg" alt="Ajouter au panier" style="vertical-align: middle; margin-right: 8px;">' . __('Ajouter au panier', 'woocommerce') . '</a>';
            echo '</div>'; // col-12
