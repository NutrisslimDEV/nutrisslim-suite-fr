...
function trusted_get_reviews_block_a() {
    ...
    echo '<h2 class="trusted-review-title">Avis des clients</h2>';
    ...
    echo '<div class="trusted-review-text">Incroyable</div>';
    ...
    foreach ($comments as $comment) {
        $rating = intval(get_comment_meta($comment->comment_ID, 'rating', true));
        $comment_content = !empty(trim($comment->comment_content)) ? esc_html($comment->comment_content) : esc_html__('Le client a laissé uniquement une évaluation.', 'woocommerce');
...
function trusted_get_reviews_block_b() {
    ...
    */
    ...
    foreach ($comments as $comment) {
		...
		$comment_content = !empty(trim($comment->comment_content)) ? esc_html($comment->comment_content) : esc_html__('Le client a laissé uniquement une note.', 'woocommerce');
        ...
        echo '<div class="trusted-review-date">' . sprintf( 
            esc_html__( '%s cela par %s', 'nutrisslim-suite' ), 
            esc_html( $time_diff ), 
            htmlspecialchars( $comment->comment_author, ENT_QUOTES, 'UTF-8' ) 
        ) . '</div>';
        echo '<div class="trusted-revComment">';
        echo '<p class="threerows">' . $comment_content . '</p><p class="rmlink"><a href="#">' . esc_html__( 'Lire la suite', 'woocommerce' ) . '</a></p>';
        echo '</div>';
        echo '</div>'; // Close trusted-revSlid
        echo '</div>'; // Close swiper-slide
    }
    ...
    if ($best_review) {
        ...
        echo '<p>' . esc_html__('De', 'woocommerce') . ' ' . esc_html($best_review->comment_author) . '</p>';
...
function trusted_get_reviews_block_c() {
    ...
    if (empty($manual_comments)) {
        return '<p>Aucun avis pour le moment. Soyez le premier à évaluer ce produit!</p>';
    }
	...
	if ($best_review) {
		...
		echo '<div class="highlighted-review-author">';
		echo '<p><strong>' . esc_html__('Przejrzane przez', 'woocommerce') . ' ' . esc_html($best_review->comment_author) . '</strong></p>';
		$nutri_leaf_url = plugin_dir_url(__FILE__) . 'assets/nutri-leaf.svg';
		echo '<div class="extra-review-logo"><img src="' . esc_url($nutri_leaf_url) . '" alt="logo" /></div>';
		echo '</div>'; // Close highlighted-review-author
		echo '</div>'; // Close highlighted-review
