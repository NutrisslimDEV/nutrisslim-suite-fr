...
function add_content_to_footer_on_product_page() {
    if (is_product()) {
        ...
        echo '<h3 id="subscrModalTitle">' . __('Économisez grâce à l\'abonnement', 'nutrisslim-suite') . '</h3>';  
        echo '<h4>' . __('Avantages de l\'abonnement', 'nutrisslim-suite') . '</h4>';
        echo '<ul>';
        echo '<li>' . __('Si vous vous inscrivez aujourd’hui, une remise supplémentaire de 5 % sera appliquée au prix d’achat unique.', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('Gagnez du temps - recevez des livraisons chaque mois sans avoir à passer commande.', 'nutrisslim-suite') . '</li>';
        //echo '<li>' . __('<strong>Free gift</strong> with every subscribed shipment, starting from the second shipment.', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('L’abonnement peut être annulé à tout moment depuis votre compte, dans la section -> Commandes -> Zone d\'abonnement, ou simplement par e-mail.', 'nutrisslim-suite') . '</li>';
        // echo '<li>' . __('<strong>Free shipping</strong> from the second shipment onwards.', 'nutrisslim-suite') . '</li>';        
        echo '</ul>';
        echo '<p>' . __('Avant l\'envoi du colis, vous recevrez un e-mail de rappel.', 'nutrisslim-suite') . '</p>';
        echo '</div>';
        echo '</div>';
    }
}
